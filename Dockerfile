...
ENV AGENT_KEY_PATH=/home/agent-admin/agent-app/api_keys/secret.key
...
# 공개키를 컨테이너 내 authorized_keys로 복사
...
RUN chmod 750 /usr/local/bin/agent-log-retention.sh && \
    ...
    chown agent-admin:agent-core $AGENT_KEY_PATH && chmod 640 $AGENT_KEY_PATH && \
