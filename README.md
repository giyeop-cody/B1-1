...
- 자동화 스크립트는 Bash로만 작성 (Python 등 대체 금지)
- 필요한 경우에만 sudo 사용 (가능한 일반 계정으로 진행)
- 제공된 Python 앱은 실행 대상이며, 과제 핵심은 관제/자동화 스크립트 구현
...
프로세스는 실행 중인데 포트가 열리지 않는 상황

---

### 1. `ss` 명령어로 상태 확인

먼저 프로세스가 실제로 어떤 포트를 잡으려 하는지, 혹은 이미 바인딩된 다른 프로세스가 있는지 확인합니다.

* **현재 리스닝 중인 포트 확인:**
```bash
ss -tulpn | grep <프로세스명 또는 포트번호>

```


* `-tulpn`: TCP/UDP, 리스닝 상태, 프로세스 ID, 숫자 주소 출력
* 만약 여기서 아무것도 나오지 않는다면, 프로세스가 **네트워크 소켓을 생성(bind)하는 단계에 도달하지 못했거나 실패**한 것입니다.



### 2. 설정 파일 및 포트 할당 확인

프로세스가 의도한 포트가 아닌 다른 포트를 쓰고 있거나, 설정 파일 로드에 실패했는지 확인합니다.

* **설정 파일 검증:** 애플리케이션의 설정 파일(`config.yaml`, `app.conf` 등)에 적힌 포트 번호가 올바른지 확인하세요.
* **포트 중복 확인:**
```bash
lsof -i :<포트번호>
...
* 다른 프로세스가 해당 포트를 이미 점유 중이라면 `bind` 에러가 발생하며 프로세스가 포트를 열지 못합니다.



### 3. `strace`로 문제 원인 추적

위의 방법으로 원인이 나오지 않는다면, 시스템 호출(System Call) 단계를 추적하여 프로세스가 왜 포트를 못 여는지 확인합니다.
...
* `-f`: 자식 프로세스까지 추적 (Fork된 프로세스 확인)
* `-e trace=network,file`: 네트워크 관련(`socket`, `bind`, `listen`, `connect`) 및 파일 관련(`openat`, `access`) 시스템 호출만 필터링


* **주요 확인 포인트:**
* `EACCES (Permission denied)`: 1024 이하의 Well-known 포트를 쓰려 하거나, SELinux/AppArmor 정책에 의해 네트워크 접근이 차단됨.
* `EADDRINUSE (Address already in use)`: 이미 해당 주소/포트가 사용 중.
* `ENOENT`: 설정 파일이나 필요한 라이브러리 파일을 찾지 못함.
* `bind(...) = -1`: `bind` 호출 자체가 실패하는 지점을 찾아 에러 코드 확인.
...
1. **`ss`**: 프로세스가 포트를 바인딩했는지, 상태는 무엇인지 확인.
2. **`lsof`**: 해당 포트를 누군가 이미 차지하고 있는지 확인.
3. **`strace`**: `bind` 또는 `listen` 시스템 콜이 실패하는 이유(에러 코드)를 실시간으로 확인.

이 외에도 **방화벽(iptables/ufw)** 설정이 포트를 차단하고 있는지(다만, 이 경우 프로세스는 포트를 열고 있을 확률이 높음) 혹은 **SELinux**가 프로세스의 네트워크 소켓 생성을 거부하고 있지는 않은지 `dmesg | grep -i avc` 명령어로 확인해보는 것도 좋습니다.

현재 점검 중인 환경에서 가장 의심되는 에러 메시지가 있다면 무엇인가요?
