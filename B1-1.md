...
- SSH 포트 변경과 Root 원격 접속 차단이 왜 기본 보안에 해당하는지
- UFW 또는 firewalld 중 하나를 선택해 “필요 포트만 허용”하는 방화벽 정책을 구성하고 검증하는 방법
- 역할 기반 계정/그룹과 ACL을 통해 “공유 디렉토리”와 “보안 디렉토리”를 분리하는 이유
- 환경 변수(AGENT_HOME 등)로 실행 환경을 고정하는 이유와 검증 방법
- 쉘 스크립트로 프로세스/포트/리소스 상태를 수집하고, 로그로 남겨 운영 문제를 추적하는 흐름
- crontab으로 모니터링을 주기 실행시키고, 로그 보존 정책(압축/삭제)이 왜 필요한지
...
- sshd 설정 파일에서 Port / PermitRootLogin 확인
...
- 앱이 `0.0.0.0:15034`로 LISTEN 상태여야 한다.
...
- 프로세스: `agent-app`(또는 제공 앱 파일명) 실행 상태를 확인하고, 비정상 시 `exit 1`
...
아래는 정답이 아니라 참고 예시입니다. 실제 문구와 구성은 달라도 됩니다.
